...
2. Card entry → `1` (clock-in proves attendance; leave conflict visible via UI warning badge)
3. Leave request found → granular `excelCode` from `leaveTypes.ts` mapping
4. Weekend → `R`
5. Otherwise → `0`
